...
// Check if any product in the order has custom checkout enabled
...
<div style="margin-bottom: <?php echo $email_improvements_enabled ? '24px' : '40px'; ?>;">
    <table class="td font-family <?php echo esc_attr($order_table_class); ?>" cellspacing="0" cellpadding="6"
        ...
        <?php if (!$email_improvements_enabled) { ?>
            <thead>
                <tr>
                    ...
                    <?php if (!$has_custom_checkout): ?>
                        <th class="td" scope="col" style="text-align:<?php echo esc_attr($text_align); ?>;">
                            <?php esc_html_e('Quantity - MA', 'woocommerce'); ?>
                        </th>
                        <th class="td" scope="col" style="text-align:<?php echo esc_attr($text_align); ?>;">
                            <?php esc_html_e('Price', 'woocommerce'); ?>
                        </th>
                    <?php else: ?>
                        <th colspan="2" class="td" scope="col" style="text-align:<?php echo esc_attr($text_align); ?>;">
                            <?php esc_html_e('Price', 'woocommerce'); ?>
                        </th>
                    <?php endif; ?>
                </tr>
        ...
        <tfoot>
            ...
            // Remove subtotal if custom checkout is enabled
            if ($has_custom_checkout && isset($item_totals['cart_subtotal'])) {
